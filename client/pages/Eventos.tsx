...
export default function Eventos() {
  ...
  const { eventos: baseEventos, eventosdoCurso, refetchEventos, deleteEvento } = useEvents();
  ...
  const eventos = useMemo(() => {
    const filteredByRole = isStudent ? eventosdoCurso : baseEventos;
    return filteredByRole.filter((e) =>
      (!q || e.titulo.toLowerCase().includes(q.toLowerCase())) &&
      (!status || e.status === status) &&
      (!responsavel || e.responsavel.toLowerCase().includes(responsavel.toLowerCase())) &&
      (!data || e.data === data)
    );
  }, [q, status, responsavel, data, isStudent, baseEventos, eventosdoCurso]);

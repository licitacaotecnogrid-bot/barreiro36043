...
function BadgeStatus({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${map[status]}`}
    >
...
export default function Dashboard() {
  ...
  const sortedEventos = [...eventosData].sort(
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-1 sm:gap-2">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground">
          Principais notícias e eventos - Curso de{" "}
          {currentUser?.curso || "Análise e Desenvolvimento de Sistemas"}
        </p>
      </div>
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                <p className="inline-flex items-center gap-1 sm:gap-2 text-xs sm:text-sm font-medium">
                  <CalendarClock className="size-3 sm:size-4" />{" "}
                  {formatDate(featuredEvent.data)}
                </p>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
        <div className="rounded-lg sm:rounded-xl border bg-card p-3 sm:p-5">
          ...
          <div className="mt-3 sm:mt-4 grid grid-cols-2 gap-2 sm:gap-3">
            <a
              href="/eventos"
              className="rounded-lg border px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm hover:bg-accent transition-colors"
            >
              Ver eventos
            </a>
            <a
        ...
        <div className="rounded-lg sm:rounded-xl border bg-card p-3 sm:p-5">
          ...
          <p className="mt-2 text-xs sm:text-sm text-muted-foreground">
            Acompanhe os principais eventos e notícias da faculdade de{" "}
            {currentUser?.curso || "Análise e Desenvolvimento de Sistemas"}.
          </p>
        </div>
      </div>
